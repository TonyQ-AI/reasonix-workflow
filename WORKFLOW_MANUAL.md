# agents-workflow — 完整技术手册 v4.1.11

> 版本：4.1.11 · 最后更新：2026-07-31
...
| 价值边界 | 编排机制是过渡性补丁：工具机制越齐全、模型越强，机械约束越冗余；方法论技能（§11）是长期资产 |

### 1.4 适用场景与价值边界

本工作流诞生于「AI 工具机制尚不完善」的环境，本质是为**工具缺机制、模型能力受限**的组合补齐短板：

| 场景 | 结论 |
|------|------|
| 工具缺少计划批准 / 任务跟踪 / 子Agent 调度 | ✅ 完整安装——工作流补齐机制缺口 |
| 模型上下文小、易遗忘、跳步、虚报完成 | ✅ 完整安装——机械流程 + 硬门控补短板 |
| 团队或新手需要标准化流程模板 | ✅ 完整安装——12 任务流水线即流程教科书 |
| 工具已有 Plan 模式 + 原生任务跟踪 + 子Agent（ZCode、Claude Code 等） | ⚠️ 只取方法论技能，跳过 wf-* 编排层 |
| 大上下文 + 强推理模型 | ⚠️ 同上——编排机制是冗余，方法论纪律仍是资产 |

**一句话**：编排机制是过渡性补丁，方法论技能是长期资产。工具机制越齐全、模型越强，需要整套工作流的地方就越少。
